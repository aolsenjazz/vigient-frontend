...
class JobsService {
  static async getAllJobs(limit: number, offset: number): Promise<JobDTO[]> {
  ...
  static async getJobById(id: number): Promise<JobDTO> {
    const response = await client.get(`/jobs/${id}`);

    return new JobDTOImpl(response.data);
  }

  static async createJob(jobData: Omit<JobDTO, "id">): Promise<JobDTO> {
